...
export default function ServerDetailStatusBox({ now, server }: { now: number; server: NezhaServer }) {
  ...
  const rings = useMemo(() => {
    ...
    return list
  }, [cpuCoreText, diskPercent, diskTotal, memPercent, memTotal, swapPercent, swapTotal, server.state?.cpu, server.state?.disk_used, server.state?.mem_used, server.state?.swap_used])
