...
type TooltipMode = "latency" | "series"
...
function defaultRightAxisLabelFormatter(value: number) {
  return `${Math.round(value)}%`
}

function defaultTooltipValueFormatter(value: number | null) {
...
export default function MiniLineChart({
  ...
  const xRange = useMemo(() => {
    const rangeMin = normalizeTimestampMs(Number(timeRange?.min))
    const rangeMax = normalizeTimestampMs(Number(timeRange?.max))
    if (rangeMin && rangeMax && rangeMax > rangeMin) {
      return { xMin: rangeMin, xMax: rangeMax }
    }
    ...
    return { xMin, xMax }
  }, [dateList, normalizedSeries, timeRange?.max, timeRange?.min])
  ...
  const timeTicks = useMemo(() => {
    ...
    return ticks
  }, [containerWidth, normalizedDateList.length, timeRange, viewWidth, xRange])
  ...
  const tooltipData = useMemo(() => {
    ...
    if (tooltipMode === "series") {
      const rows: TooltipRow[] = normalizedSeries.map((s, idx) => {
        ...
        return {
          ...
          name: s.name,
          color: s.color,
          value: tooltipValueFormatter(value, s),
        }
      })
    ...
    const rows: TooltipRow[] = Array.from(map.entries()).map(([key, v]) => {
      ...
      return { key, name: v.name, color: v.color, value: `延时 ${delayText} 丢包 ${lossText}` }
    })

    return { time: t, label, rows, span }
  }, [hoverIdx, hovering, normalizedDateList, normalizedSeries, tooltipMode, tooltipValueFormatter, valueMapList, xRange.xMax, xRange.xMin])
      ...
      <svg className="chart" viewBox={`0 0 ${viewWidth} ${viewHeight}`} preserveAspectRatio="none">
        ...
        <g className="chart-axis-labels">
          ...
          {hasLossSeries &&
            yTicks.map((tick) => {
              ...
              const label = rightAxisLabelFormatter(p)
              return (
                <text
                  key={`yr_${tick}`}
        ...
        {paths.map((p) => (
          <g key={p.id}>
            ...
            {p.markers.map((point, index) => (
              <circle
                key={`${p.id}_marker_${index}`}
                cx={point.x}
                cy={point.y}
                r={2.4}
                fill={p.color}
