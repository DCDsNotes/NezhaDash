...
export default (params) => {
  ...
  const serverStatusColorMode = 'default';
  ...
  const useMemAndTotalMem = computed(() => {
    ...
    const totalBytes = props.info?.Host?.MemTotal || 0;
    const total = hostUtils.calcBinary(totalBytes || 1);
    const usePercent = totalBytes > 0
      ? ((props.info?.State?.MemUsed / totalBytes) * 100).toFixed(2) * 1
      : 0;
    return {
      used,
      total,
  ...
  const useSwapAndTotalSwap = computed(() => {
    ...
    const totalBytes = props.info?.Host?.SwapTotal || 0;
    const total = hostUtils.calcBinary(totalBytes || 1);
    const usePercent = totalBytes > 0
      ? ((props.info?.State?.SwapUsed / totalBytes) * 100).toFixed(2) * 1
      : 0;
    return {
      used,
      total,
  ...
  const useDiskAndTotalDisk = computed(() => {
    ...
    const totalBytes = props.info?.Host?.DiskTotal || 0;
    const total = hostUtils.calcBinary(totalBytes || 1);
    const usePercent = totalBytes > 0
      ? ((props.info?.State?.DiskUsed / totalBytes) * 100).toFixed(2) * 1
      : 0;
    return {
      used,
      total,
  ...
  const serverStatusList = computed(() => statusListTpl.split(',').map((i) => {
    ...
    switch (i) {
      ...
      case 'mem':
      {
        ...
        const usedColor = getColor('mem', serverStatusColorMode);
        return {
          type: 'mem',
      ...
      case 'swap':
      {
        ...
        const usedColor = getColor('swap', serverStatusColorMode);
        return {
          type: 'swap',
      ...
      case 'disk':
      {
        ...
        const usedColor = getColor('disk', serverStatusColorMode);
        return {
          type: 'disk',
