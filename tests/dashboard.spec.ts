...
test("dashboard interactions remain usable on desktop and mobile", async ({ page }, testInfo) => {
  ...
  const firstTabEdge = await page.locator(".status-controls__state--active").evaluate((element) => {
    ...
    return {
      indicatorLeft: tabRect.left + Number.parseFloat(indicator.left),
      panelLeft: panelRect?.left ?? Number.POSITIVE_INFINITY,
      panelOverflow: panel ? window.getComputedStyle(panel).overflow : "",
    }
  })
  ...
  const lastTabEdge = await page.locator(".status-controls__state--active").evaluate((element) => {
    ...
    const indicator = window.getComputedStyle(element, "::after")
    return {
      indicatorRight: tabRect.right - Number.parseFloat(indicator.right),
  ...
  const speedAxisLabel = speedChart.locator(".chart-axis-labels text").first()
  await expect(speedAxisLabel).toHaveAttribute("font-size", "10")
  await expect(speedAxisLabel).toHaveAttribute("font-weight", "600")
  await speedChart.hover()
  const speedTooltip = speedChart.locator(".chart-tooltip")
  await expect(speedTooltip).toBeVisible()
  await expect(speedTooltip).toHaveCSS("top", "15px")
  await expect(speedTooltip).toHaveCSS("background-color", "rgb(248, 250, 249)")
  ...
  const monitorAxisLabel = monitorChart.locator(".chart-axis-labels text").first()
  await expect(monitorAxisLabel).toHaveAttribute("font-size", "10")
  await expect(monitorAxisLabel).toHaveAttribute("font-weight", "600")
  await monitorChart.hover()
  const monitorTooltip = monitorChart.locator(".chart-tooltip")
  await expect(monitorTooltip).toBeVisible()
  await expect(monitorTooltip).toHaveCSS("top", "15px")
  await expect(monitorTooltip).toHaveCSS("background-color", "rgb(248, 250, 249)")
